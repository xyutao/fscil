...
from model.resnet18_v1 import resnet18_v1

from gluoncv.utils import makedirs
from gluoncv.data import transforms as gcv_transforms

from dataloader.dataloader import NC_MiniImageNet, merge_datasets, merge_mini_imagenet
from tools.ng_anchor import prepare_anchor

from tools.utils import plot_schedule, LinearWarmUp,DataLoader
from tools.loss import  NG_Min_Loss,NG_Max_Loss
...
#####init CNN
        if opt.dataset == 'NC_MINI_IMAGENET':
            n = mx.nd.zeros(shape=(1,3,84,84),ctx=ctx[0])
        else:
...
# ==========================================================================
    ...
    for sess in sesses:
        ...
        # plot_schedule(schedule, epochs[sess])
                ...
                if ng_update == True:
                    if sess==1:
                        ...
                        new_set = DATASET(train=True, fine_label=True, fix_class=new_class, base_class=base_class,
                                    logger=logger)
                        update_anchor2 = merge_datasets(update_anchor, new_set)
        ...
        if model_name == 'resnet18' and opt.dataset == 'NC_MINI_IMAGENET' and opt.small_lr:
            net.collect_params('.*resnetv10_conv0_weight').setattr('lr_mult',0)
            net.collect_params('.*stage').setattr('lr_mult',0.001)
        ...
        if use_nme:
            ...
            digG_weight = opt.nme_weightmini_merge_datasets
        if use_AL:
            if opt.ng_var:
        ...
        for epoch in range(epochs[sess]):
            ...
            train_disg_loss, train_pdl_loss , train_min_loss ,train_max_loss = 0, 0 , 0,0
            ...
            for i, batch in enumerate(train_data):
                ...
                with ag.record():
                    ...
                    if sess > 0 and use_pdl:
                        ...
                        anchor_l = [anchor_l[0][:,:60+(sess-1)*c_way]]
                        soft_label = [mx.nd.softmax(anchor_logit[0][:,:60+(sess-1)*c_way] / temperature)]
